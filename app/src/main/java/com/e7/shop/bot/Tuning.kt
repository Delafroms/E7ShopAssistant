...
object Tuning {
    ...
    /**
     * 弹窗内容区的纵向窗口（相对弹窗锚点的画面高度占比）。
     *
     * **为什么需要**：购买弹窗是**盖在商店列表上的对话框**，弹窗外的列表行仍会被
     * OCR 完整读到。2026-09-20 生产日志实测：玩家要点的那件誓约书签就在弹窗下方
     * 仍可见，于是整帧里 184000 出现两次（弹窗内一次 + 弹窗外列表行一次），
     * 「价格唯一匹配」判据 100% 失败 → 一夜 2472 次「买 → 取消 → 再买」活锁、
     * 只买到 8 件、4.4 小时满速空转（手机严重发烫）。三重验证必须只看**弹窗内容区**。
     *
     * **取值依据**（OnePlus PLR110 2800×1272 生产日志实测）：
     *  · 弹窗按钮（cancel/confirm）y ≈ 913  ← 锚点
     *  · 弹窗内商品图标 y ≈ 623             ← 向上 0.228h，故向上取 0.32 留余量
     *  · 弹窗外列表行 y ≈ 1130              ← 向下 0.173h，故向下必须 < 0.173
     * 向下 0.08 是**关键约束**：它必须小于弹窗外列表行的距离，否则列表行又会被算进来。
     */
    const val DIALOG_WIN_UP = 0.32f
    const val DIALOG_WIN_DOWN = 0.08f
     ...
     * 判定"两帧几乎完全相同"的采样与阈值。
     *
     * **为什么需要**（2026-09-20 功耗优化）：同一画面被识别两次是纯浪费。
     * 生产日志实测 AI 引擎的 OBSERVE 与 DECIDE_TARGETS 分别消耗 2489 / 2529 帧
     * （占全部完整识别的 40%），而两者之间**没有任何动作**，画面不可能变；
     * 弹窗探测帧与紧随其后的"稳定帧"同理。复用同一帧的识别结果可省下这一次
     * OCR（354ms）+ YOLO（241ms）。
     *
     * **判定必须宁可判为"不同"**：判不同只是多花一次识别（行为与优化前完全一致）；
     * 判相同却判错，就会拿旧结果做决策 —— 那是漏买/误买的来源。所以阈值取得很小，
     * 且任何异常（尺寸不符 / 比对失败）一律返回"不同"。
     */
    /** 横向采样步长（像素）。 */
    const val FRAME_IDENTICAL_STEP = 4
    /** 纵向采样行距（像素）。 */
    const val FRAME_IDENTICAL_ROW_STEP = 4
    /** 单像素视为"相同"的三通道差之和上限（抗截图噪声）。 */
    const val FRAME_IDENTICAL_PIXEL_DIFF = 12
    /** 允许的差异像素比例上限，超过即判为"画面变了"→ 重新识别。 */
    const val FRAME_IDENTICAL_RATIO = 0.002f
    ...
    /**
     * 刷新「点下但列表没变」的连续上限（2026-09-22 第二轮红队 R1）。
     *
     * 攻击场景：刷新确认键点不中（点到取消 / 坐标偏）→ 弹窗关了但列表没换 →
     * 引擎记账（保守方向对）→ 判未验证 → 退避 → 回 SCAN → 又刷新：一圈一圈。
     * 实测在测试的 400 帧预算内就刷了 44 次、记账 132 颗天空石；真机会一直烧到
     * maxSkystones 预算耗尽。
     *
     * 达到上限即停机并报错：连续失败说明是系统性问题（点不中 / 网络异常），
     * 继续循环只会把天空石或挂机时间全部吃掉。
     */
    const val REFRESH_UNVERIFIED_MAX_STREAK = 5
